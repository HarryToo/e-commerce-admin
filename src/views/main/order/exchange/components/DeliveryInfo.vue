<template>
  <el-form ref="form" :model="formData" :rules="formRules" size="small" label-position="right" label-width="166px"
           label-suffix=":">
    <el-form-item label="返回商品收货人" prop="name">
      <el-input v-model="formData.name" placeholder="请输入收货人姓名"></el-input>
    </el-form-item>
    <el-form-item label="返回商品收货人手机号" prop="phone">
      <el-input v-model="formData.phone" placeholder="请输入收货人手机号"></el-input>
    </el-form-item>
    <el-form-item label="返回商品收货地址" prop="area">
    ...
    <el-form-item label="返回商品详细地址" prop="address">
    ...
    <el-form-item align="right" style="margin-bottom: 0;">
      <el-button class="custom" @click="submit">同意换货</el-button>
    </el-form-item>
  </el-form>
</template>
  ...
  setup(props) {
    ...
    const submit = async () => {
      if (await form.value.validate()) {
        const {code} = await $api.orderApi.exchange.review(resultData.value)
        if (code === 200) {
          ElMessage.success('已同意换货')
          closeDialog()
          refreshData()
        }
      }
