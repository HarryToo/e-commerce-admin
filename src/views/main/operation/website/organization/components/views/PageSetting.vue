<template>
  <div class="page-edit">
    <div class="top">
      <div class="last-update-time text-cut-1">最近保存时间：{{
          $store.state.decoration.organizationWebsite.updateTime || '无'
        }}
      </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const floorComponent = computed(() => {
      ...
      const floorType = store.state.decoration.organizationWebsite.homepage.floor[floorIndex.value].type
      return compNameArr[floorType - 1]
    })

    // 保存配置
    const saveConfigs = () => {
      ElMessageBox.confirm('请确保必要的配置完整且正确，确认保存？', '保存网站装修（机构版）配置', {type: 'info'}).then(async () => {
        const {code} = await store.dispatch('decoration/organizationWebsite/saveAllConfigs')
        if (code === 200) {
          ElMessage.success('网站装修（机构版）配置保存成功')
        }
      }).catch(() => {
      })
    }
    ...
    const publishConfigs = async () => {
      ElMessageBox.confirm('此操作将正式启用当前配置，请确保必要的配置完整且正确，确认发布？', '发布网站装修（机构版）配置', {type: 'warning'}).then(async () => {
        const {code} = await store.dispatch('decoration/organizationWebsite/publishAllConfigs')
        if (code === 200) {
          ElMessage.success('网站装修（机构版）配置发布成功')
        }
      }).catch(() => {
      })
    }
