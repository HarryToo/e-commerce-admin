	...
	<el-dialog title="批量设置库存：" v-model="price.StockDialog.state">
		<div class="DiaLog">
			<el-radio v-model="price.StockDialog.radio" label="1">
				<div>库存统一设置为</div>
				<el-input v-model="price.StockDialog.input1" :disabled="price.StockDialog.radio != 1"
					placeholder="请输入内容"></el-input>
			</el-radio>
			<el-radio v-model="price.StockDialog.radio" label="2">
				<div>原库存基础上增加</div>
				<el-input v-model="price.StockDialog.input2" :disabled="price.StockDialog.radio != 2"
					placeholder="请输入内容"></el-input>
			</el-radio>
			<el-radio v-model="price.StockDialog.radio" label="3">
				<div>原库存基础上减少</div>
				<el-input v-model="price.StockDialog.input3" :disabled="price.StockDialog.radio != 3"
					placeholder="请输入内容"></el-input>
			</el-radio>
		</div>
		<template #footer>
	...
	<el-dialog title="批量设置销售价：" v-model="price.PriceDialog.state">
		<div class="DiaLog">
			...
			<el-radio v-model="price.PriceDialog.radioTp" label="2">
				<div>设置统一价</div>
				<el-input v-model="price.PriceDialog.inputPrice" :disabled="price.PriceDialog.radioTp != 2"
			...
			<el-radio v-model="price.PriceDialog.radioTp" label="3">
				...
				<div class="PriceDialogCopt">
					<div>
						来源价*
						<el-input size="mini" v-model="price.PriceDialog.formula.percent"
							:disabled="price.PriceDialog.radioTp != 3"></el-input>%+
						<el-input size="mini" v-model="price.PriceDialog.formula.add"
							:disabled="price.PriceDialog.radioTp != 3"></el-input> -
						<el-input size="mini" v-model="price.PriceDialog.formula.subtract"
							:disabled="price.PriceDialog.radioTp != 3">></el-input>
					</div>
					<div style="display: flex; align-items: center; margin-top: 1rem;">
						<div>价格取整&nbsp;&nbsp;&nbsp;</div>
						<el-radio v-model="price.PriceDialog.rounding" label="1"
							:disabled="price.PriceDialog.radioTp != 3">四舍五入</el-radio>
						<el-radio v-model="price.PriceDialog.rounding" label="2"
							:disabled="price.PriceDialog.radioTp != 3">向上取整</el-radio>
						<el-radio v-model="price.PriceDialog.rounding" label="3"
							:disabled="price.PriceDialog.radioTp != 3">向下取整</el-radio>
					</div>
					<div style="display: flex; align-items: center;margin-top: 1rem;">
						<div>保留小数&nbsp;&nbsp;&nbsp;</div>
						<el-radio v-model="price.PriceDialog.retain" label="1"
							:disabled="price.PriceDialog.radioTp != 3">保留1位小数</el-radio>
						<el-radio v-model="price.PriceDialog.retain" label="2"
							:disabled="price.PriceDialog.radioTp != 3">保留2位小数</el-radio>
					</div>
				</div>
			</el-radio>
		</div>
				...
				this.goods_specification.forEach(function(item, dex) {
					if (item.IsUse) {
						if (_this.price.StockDialog.radio == 1) {
							if (!_this.price.StockDialog.input1) {
								return;
							}
							item.inventory = Number(_this.price.StockDialog.input1)
						} else if (_this.price.StockDialog.radio == 2) {
							if (!_this.price.StockDialog.input2) {
								return;
							}
							item.inventory = Number(_this.price.StockDialog.input2) + Number(item
								.inventory)
						} else if (_this.price.StockDialog.radio == 3) {
							if (!_this.price.StockDialog.input3) {
								return;
							}
							item.inventory = Number(item.inventory) - Number(_this.price.StockDialog
								.input3)
				...
				this.goods_specification.forEach(function(item, dex) {
					if (item.IsUse) {
						if (_this.price.PriceDialog.radioTp == '1') { //同源
							...
							var Cprice = Number(item.source_price) * Number(_this.price.PriceDialog.formula
								.percent) / 100 + Number(_this.price.PriceDialog.formula.add) - Number(
								_this.price.PriceDialog.formula.subtract);
